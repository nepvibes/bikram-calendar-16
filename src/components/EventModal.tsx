...
const EventModal: React.FC<EventModalProps> = ({
  ...
  return <Dialog open={isOpen} onOpenChange={() => handleClose()}>
      <DialogContent className="sm:max-w-md overflow-hidden shadow-xl rounded-xl bg-pink-200">
        <div className={`bg-gradient-to-br from-blue-800 to-blue-600 dark:from-blue-900 dark:to-blue-800 -mx-6 -mt-6 p-4 text-white transition-all duration-300 rounded-t-xl ${isAnimating ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`}>
        ...
        <div className="space-y-4 py-4">
          <div className={`flex flex-col space-y-4 text-center transition-all duration-300 delay-200 ${isAnimating ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`}>
            ...
            {eventData.eventText && <div className={`mt-4 pt-4 border-t transition-all duration-300 delay-300 ${isAnimating ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`}>
